...
__global__ void  mandelbrot(int Nre, int Nim, complex_t cmin, complex_t cmax, float *count){ 
  ...
  complex_t c;
   ...
   int m = tIdx + bIdx*bSizex; //unique x coordinate
  int n = tIdy + bIdy*bSizey; //unique y coordinate  double dr = (cmax.r-cmin.r)/(Nre-1);
...
float *count1;
cudaMalloc(&count1,Nre*Nim*sizeof(float));
