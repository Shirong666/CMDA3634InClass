...
MPI_Send(message, Nmessages,MPI_UNSIGNED,0,66,MPI_COMM_WORLD);
MPI_Send(a,Nmessages,MPI_UNSIGNED,0,666,MPI_COMM_WORLD);
}
  /* Q2.3 Have Alice recv all the encrypted mesages 
    from Bob (rank 1) and then decrypt them */
if(rank == 0)
{
MPI_Recv(message, Nmessages,MPI_UNSIGNED, 1,66,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
MPI_Recv(a,Nmessages,MPI_UNSIGNED,1,666,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
